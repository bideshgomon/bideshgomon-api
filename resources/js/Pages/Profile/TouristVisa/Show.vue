...
const saveChanges = async () => {
    ...
    try {
        await axios.put(route('api.tourist-visa-applications.update', props.application.id), form.value);
...
const deleteApplication = async () => {
    if (confirm('Are you sure you want to delete this application? This action cannot be undone.')) {
        try {
            await axios.delete(route('api.tourist-visa-applications.destroy', props.application.id));
            router.visit(route('profile.tourist-visa.index'));
        } catch (error) {
...
<template>
    <Head title="Tourist Visa Application Details" />
