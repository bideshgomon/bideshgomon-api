...
const fetchApplications = async (page = 1) => {
    loading.value = true;
    try {
        const response = await axios.get(route('api.work-visa-applications.index'), { // Use API route
            params: { page: page }
        });
...
<template>
    <Head title="My Work Visa Applications" />

    <AuthenticatedLayout>
        <template #header>
            <h2 class="font-semibold text-xl text-gray-800 leading-tight">My Work Visa Applications</h2>
        </template>
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 text-gray-900">
                        <div class="mb-4 flex justify-end">
                            <Link :href="route('profile.work-visa.create')" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150">
                                Apply for Work Visa
                            </Link>
                        </div>
                                ...
                                <tbody class="bg-white divide-y divide-gray-200">
                                    <tr v-if="loading">
                                     ...
                                     <tr v-else-if="applicationList.length === 0">
                                         <td colspan="5" class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 text-center">No applications found.</td>
                                    </tr>
                                    <tr v-else v-for="app in applicationList" :key="app.id">
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ app.job_category?.name ?? 'N/A' }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 capitalize">{{ app.status }}</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <Link :href="route('profile.work-visa.show', app.id)" class="text-indigo-600 hover:text-indigo-900">View Details</Link>
                                            </td>
                                    </tr>
                                </tbody>
                            </table>
