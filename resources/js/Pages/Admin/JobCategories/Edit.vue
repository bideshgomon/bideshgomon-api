...
const submit = () => {
    form.put(route('api.admin.job-categories.update', props.category.id));
};

const deleteCategory = () => {
    if (confirm('Are you sure you want to delete this category?')) {
        router.delete(route('api.admin.job-categories.destroy', props.category.id), {
             onSuccess: () => router.visit(route('admin.job-categories.index')), // Redirect after delete
        });
    }
};
</script>
...
<template>
    <Head :title="'Edit Job Category: ' + category.name" />

    <AdminLayout>
        <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
            <section>
                <header class="mb-6">
                    <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">Edit Job Category: {{ category.name }}</h2>
                ...
                <form @submit.prevent="submit" class="space-y-6 max-w-xl">
                    ...
                    <div class="flex items-center justify-between gap-4">
                        <div class="flex items-center gap-4">
                            <PrimaryButton :disabled="form.processing">{{ form.processing ? 'Saving...' : 'Update Category' }}</PrimaryButton>
                        ...
                        <DangerButton @click="deleteCategory" type="button" :disabled="form.processing">Delete Category</DangerButton>
                    </div>
                </form>
            </section>
        </div>
