            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="mb-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h1 class="text-3xl font-bold text-gray-900 dark:text-white">Countries Management</h1>
                            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">Manage all countries in the system</p>
                        </div>
                        <div class="flex items-center space-x-3">
                            <Link
                                :href="route('admin.data.countries.bulk-upload')"
                ...
                <div class="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 mb-6">
                    ...
                    <div class="flex items-center justify-between mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                        ...
                        <a
                            :href="route('admin.data.countries.export', form)"
                        ...
                        <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-700">
                                <tr>
                                    ...
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">ISO Codes</th>
                                    <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-300 uppercase tracking-wider">Phone Code</th>
                            ...
                            <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                <tr v-for="country in countries.data" :key="country.id" class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 dark:text-white">{{ country.nationality || '—' }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="text-sm text-gray-900 dark:text-white">{{ country.iso_code_2 }} / {{ country.iso_code_3 }}</div>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ country.phone_code }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white">{{ country.currency_code }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">{{ country.region || '—' }}</td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <button
                                            @click="toggleStatus(country)"
                                            :class="country.is_active ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-300'"
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        <div class="flex items-center justify-end space-x-2">
                                            <Link
                                                :href="route('admin.data.countries.edit', country.id)"
...
const filter = () => {
    router.get(route('admin.data.countries.index'), form, {
        preserveState: true,
        preserveScroll: true,
    });
};
...
const sortBy = (field) => {
    ...
    router.get(route('admin.data.countries.index'), {
        ...form,
        sort: field,
        direction,
    }, {
...
const toggleStatus = (country) => {
    if (confirm(`Are you sure you want to ${country.is_active ? 'deactivate' : 'activate'} ${country.name}?`)) {
        router.post(route('admin.data.countries.toggle-status', country.id), {}, {
            preserveScroll: true,
        });
    }
};
...
const deleteCountry = (country) => {
    if (confirm(`Are you sure you want to delete ${country.name}? This action cannot be undone.`)) {
        router.delete(route('admin.data.countries.destroy', country.id), {
            preserveScroll: true,
        });
    }
};
